...
std::deque<std::string> sort(std::deque<std::string> filtered);
...
void map3(std::deque<std::string> deque){
    if(*Graceful_Exit == false){
        std::cout << "Data filtered done! Create forks and separating the words list..." << std::endl;
    }
    ...
    for(int i = 1; i < 14; i++){
        ...
        if(sort_fork == 0){
            ...
            while(deque.size() != 0 && *Graceful_Exit == false){
                ...
                // i+3 is the length of word that list include
                if((int)word.length() == (i + 2)){
                    word_list[i]->push_back(word);
                }
