...
std::deque<std::string> TaskFilter(std::deque<std::string> original_data);
...
int main(int argc, char **argv) {
    ...
    words_in_original_data = (int *)mmap(NULL, 4, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, -1, 0);
    ...
    if(Graceful_Exit == MAP_FAILED || words_in_3 == MAP_FAILED || words_in_4 == MAP_FAILED || words_in_5 == MAP_FAILED
         ...
         || words_in_original_data == MAP_FAILED){
        std::cerr << "mmap fail!" << std::endl;
        exit(0);
    }
    *Graceful_Exit = false;
    *sort_done = 0;
    *words_in_original_data = 0;
    ...
    if(main_fork != 0){
        ...
        std::cout << "Checking output file name..." << std::endl;
        ...
        if(*Graceful_Exit == false){
            std::cout << "Output file name exist, read input Stream..." << std::endl;
        }

        std::deque<std::string> output;
        // create forks to get input stream to map5 by FIFO
        pid_t map5_fork = fork();
        if(map5_fork < 0){
            std::cerr << "Create fork fail!" << std::endl;
            exit(0);
        }
        // stream server is child
        if(map5_fork == 0){
            // get input stream
            std::deque<std::string> original_data;

            // read input Stream
            std::string word_temp;
            while(std::cin >> word_temp){
                original_data.push_back(word_temp);
                *words_in_original_data = *words_in_original_data + 1;
            }

            if(*Graceful_Exit == false){
                std::cout << "All words read, sending to map5()..." << std::endl;
            }
            
            // use stream server send word to map5() by FIFO
            stream_server(original_data);
            exit(0);
        }
        // map5 is parent
        else{
...
std::deque<std::string> TaskFilter(std::deque<std::string> original_data){
    std::cout << "All words readed, start filtering data..." << std::endl;
    std::deque<std::string> filtered;

    while(original_data.size() != 0 && *Graceful_Exit == false){
        std::string temp = original_data.at(0);
        if(temp.length() >= 3 && temp.length() <= 15){
            filtered.push_back(temp);
        }

        original_data.pop_front();
    }
...
std::deque<std::string> my_sort(std::deque<std::string> filtered){
    ...
    if(filtered_sorted.size() != 0 && *Graceful_Exit == false){
...
void stream_server(std::deque<std::string> original_data){
    // send word randomly to FIFO
    // check FIFO file exist,, if not create FIFO file
    char fifo_name[] = "FIFO_stream_server";
    int fifo = access(fifo_name, F_OK);
    if(fifo != 0){
        fifo = mkfifo(fifo_name, 0777);
        if(fifo < 0){
    ...
    // write words randomly
    while(original_data.size() != 0 && *Graceful_Exit == false){
        // reason of not use random is in report
        int random_i = 0;
        //int random_i = rand() % original_data.size();
        std::string word = original_data.at(random_i);
        
        // write length of word
        int current_word_length = word.length();
        char current_word_length_char[10];
        snprintf(current_word_length_char, sizeof(current_word_length_char), "%d", current_word_length);
        write(fifo, current_word_length_char, 10);

        // write word
        char word_char[current_word_length];
        std::strcpy(word_char, word.c_str());
        write(fifo, word_char, sizeof(word_char)+1);

        // update deque
        std::deque<std::string>::iterator filtered_i = original_data.begin() + random_i;
        original_data.erase(filtered_i);
    }
...
std::deque<std::string> map5(){
    ...
    // read words in FIFO send by stream server
    // check FIFO file exist,, if not create FIFO file
    char fifo_name[] = "FIFO_stream_server";
    int fifo = access(fifo_name, F_OK);
    if(fifo != 0){
    ...
    // read words
    while(*words_in_original_data != 0 && *Graceful_Exit == false){
        // first read length of word
        char length_char[10];
        read(fifo, length_char, 10);
        int length = atoi(length_char);
        
        // read word
        char word_char[length];
        read(fifo, word_char, sizeof(word_char)+1);

        std::string word = word_char;
        original_data.push_back(word);
    ...
    filtered_data = TaskFilter(original_data);

    // create forks for map4 and reduce5, the process here is same as Task4
    std::deque<std::string> output;
